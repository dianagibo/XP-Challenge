...
async function showGoals(request, response, next) {
  try {
    ...
    return response.render('pages/weekly-goals', { pageTitle: 'Weekly goals', activePage: 'goals', goals, players, flash, error: null, values: {}, categories: service.CATEGORIES, today: service.localDateString(new Date()) });
...
async function createGoal(request, response, next) {
  try {
    ...
    request.session.flash = { type: 'success', message: 'Weekly goal created!' };
    ...
    if (error.status === 400) {
      try {
        ...
        return response.status(400).render('pages/weekly-goals', { pageTitle: 'Weekly goals', activePage: 'goals', goals, players, flash: null, error: error.message, values: request.body, categories: service.CATEGORIES, today: service.localDateString(new Date()) });
